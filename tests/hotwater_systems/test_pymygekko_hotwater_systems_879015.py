...
async def var_response(request):
    varResponseFile = open("tests/hotwater_systems/data/api_var_response_879015.json")
    return web.Response(status=200, body=varResponseFile.read())


async def var_status_response(request):
    statusResponseFile = open(
        "tests/hotwater_systems/data/api_var_status_response_879015.json"
    )
...
async def test_get_hotwater_systems(mock_server):
    ...
    async with ClientSession() as session:
        ...
        assert hotWaterSystems[0].name == "WW Boiler"
        assert hotWaterSystems[0].state == HotWaterSystemState.OFF
        assert hotWaterSystems[0].target_temperature == 48.00
        assert hotWaterSystems[0].current_temperature_bottom == 48.0
        ...
        assert (
            HotWaterSystemFeature.TOP_TEMPERATURE
            in hotWaterSystems[0].supported_features
        )
        assert (
            HotWaterSystemFeature.TARGET_TEMPERATURE
